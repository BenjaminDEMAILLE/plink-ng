...
double BlocksCalcLnlike(double known11, double known12, double known21, double known22, double center_ct_d, double freq11, double freq12, double freq21, double freq22, double half_hethet_share, double freq11_incr) {
  double lnlike;
  freq11 += freq11_incr;
  freq22 += freq11_incr;
...
uint32_t BlocksEmPhaseHethet(double known11, double known12, double known21, double known22, uint32_t center_ct, double* freq1x_ptr, double* freq2x_ptr, double* freqx1_ptr, double* freqx2_ptr, double* freq11_ptr, uint32_t* onside_sol_ct_ptr) {
  ...
  if (center_ct) {
    ...
    if (sol_end_idx > sol_start_idx + 1) {
      ...
      best_lnlike = BlocksCalcLnlike(known11, known12, known21, known22, center_ct_d, freq11, freq12, freq21, freq22, half_hethet_share, best_sol);
      cur_sol_idx = sol_start_idx + 1;
      do {
	incr_1122 = solutions[cur_sol_idx];
        cur_lnlike = BlocksCalcLnlike(known11, known12, known21, known22, center_ct_d, freq11, freq12, freq21, freq22, half_hethet_share, incr_1122);
...
PglErr HaploviewBlocks(const uintptr_t* orig_variant_include, const ChrInfo* cip, const uint32_t* variant_bps, const char* const* variant_ids, const uintptr_t* allele_idx_offsets, const double* allele_freqs, const uintptr_t* founder_info, const PhenoCol* pheno_cols, const BlocksInfo* bip, uint32_t raw_sample_ct, uint32_t raw_variant_ct, uint32_t variant_ct, uint32_t pheno_ct, uint32_t max_thread_ct, PgenReader* simple_pgrp, char* outname, char* outname_end) {
  ...
  PglErr reterr = kPglRetSuccess;
  {
    ...
    const double max_maf = 1 - min_maf;
    {
      ...
      for (uint32_t vidx = 0; vidx != variant_ct; ++vidx) {
        ...
        if (allele_idx_offsets && (allele_idx_offsets[variant_uidx + 1] - allele_idx_offsets[variant_uidx] != 2)) {
          ...
          if ((ref_freq < min_maf) || (ref_freq > max_maf)) {
            drop = 1;
          }
        }
        if (drop) {
    ...
    uint32_t* strong_cts;
    uint32_t* rec_cts;
    if (unlikely(bigstack_alloc_w(max_block_size * geno_mask_stride, &window_masks) ||
                 bigstack_alloc_u32(max_block_size, &window_uidxs) ||
                 bigstack_alloc_u32(max_block_size, &strong_cts) ||
                 bigstack_alloc_u32(max_block_size, &rec_cts))) {
      goto HaploviewBlocks_ret_NOMEM;
    }
    ...
    for (uint32_t chr_fo_idx = 0; chr_fo_idx != cip->chr_ct; ++chr_fo_idx) {
      ...
      for (uint32_t cidx = 0; cidx != chr_variant_ct; ++cidx) {
        ...
        const uint32_t cur_slot = cidx % max_block_size;
        reterr = PgrGet(founder_pnm, pssi, founder_ct, cur_uidx, simple_pgrp, genovec);
        if (unlikely(reterr)) {
        ...
        uint32_t cur_rec = 0;
        const uint32_t max_delta = cidx - window_start_cidx;
        for (uint32_t delta = 1; delta <= max_delta; ++delta) {
          const uint32_t other_cidx = cidx - delta;
          const uint32_t other_slot = other_cidx % max_block_size;
          uint32_t counts[9];
          BlocksPairCounts(&(window_masks[cur_slot * geno_mask_stride]), &(window_masks[other_slot * geno_mask_stride]), founder_ctl, counts);
          const uint32_t cur_ci_type = HaploviewBlocksClassify(counts, lowci_max, lowci_min, recomb_highci, strong_highci, strong_lowci, strong_lowci_outer, 0, recomb_fast_ln_thresh);
          if (cur_ci_type > 4) {
            ++cur_strong;
          } else if (!cur_ci_type) {
            ++cur_rec;
...
PglErr ShowTags(const uintptr_t* orig_variant_include, const ChrInfo* cip, const uint32_t* variant_bps, const char* const* variant_ids, const uintptr_t* allele_idx_offsets, const uintptr_t* founder_info, const char* tag_fname, uint32_t list_all, uint32_t mode2, uint32_t bp_radius, double r2_thresh, uint32_t raw_variant_ct, uint32_t variant_ct, uint32_t raw_sample_ct, uint32_t founder_ct, uint32_t max_variant_id_slen, uint32_t output_zst, uint32_t max_thread_ct, PgenReader* simple_pgrp, char* outname, char* outname_end) {
  ...
  PglErr reterr = kPglRetSuccess;
  {
    ...
    memcpy(variant_include, orig_variant_include, raw_variant_ctl * sizeof(intptr_t));
    // r^2 here is the biallelic unphased statistic, as in PLINK 1.x.
    uint32_t skip_ct = 0;
    if (allele_idx_offsets) {
      uintptr_t variant_uidx_base = 0;
      uintptr_t cur_bits = orig_variant_include[0];
    ...
    if (skip_ct) {
      logprintf("(%u multiallelic and/or haploid-chromosome variant%s skipped.)\n", skip_ct, (skip_ct == 1)? "" : "s");
    }
  }
  while (0) {
  ShowTags_ret_NOMEM:
